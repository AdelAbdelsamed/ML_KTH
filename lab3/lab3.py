...
def computePrior(labels, W=None):
    ...
    for jdx, class_k in enumerate(classes):
        # Obtain indices of datasets belonging to class k
        idx = np.where(labels == class_k)[0]
        X_k = X[idx,:]
        W_k = W[idx]

        # Compute N_k 
        Npts_k, Ndims_k = np.shape(X_k)

        prior[jdx] = np.sum(W_k)
...
def mlParams(X, labels, W=None):
    ...
    for jdx,class_k in enumerate(classes):
        ...
        # Obtain all data from class k
        X_k = X[idx,:]
        W_k = W[idx] # Assignment 5

        # Compute N_k 
        Npts_k, Ndims_k = np.shape(X_k)

        # Compute the mean of each 
        #mu[jdx, :] = np.mean(X_k, axis = 0)
        mu[jdx, :] = (W_k.T  @ X_k)/np.sum(W_k)


        # Compute the covaraince matrix 
        #sigma[jdx,:,:] = np.diag(np.mean(np.square(X_k - mu[jdx, :]), axis = 0)) 
        sigma[jdx,:,:] = (1/np.sum(W_k))* np.diag(W_k.T @ np.square(X_k - mu[jdx, :])) 
...
def classifyBayes(X, prior, mu, sigma):
    ...
    for i in range(Nclasses):
        for j in range(Npts):
            logProb[i, j] = -0.5*np.log(sigma[i, :, :].diagonal().prod()) + np.log(prior[i]) - 0.5*((X[j, :] - mu[i, :]) @ np.diag(1.0 / np.diag(sigma[i, :, :])) @ (X[j, :] - mu[i, :]).T)
...
#plotBoundary(BoostClassifier(BayesClassifier()), dataset='iris',split=0.8)
